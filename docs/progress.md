...
생성(결제/SSH키, 대행 불가) + 며칠 실측 경과시간이 전제라 이 세션에서 "완료"할 수
없음 — 대신 실행 가능해질 때 바로 따라갈 수 있는 런북만 준비.

- `docs/deploy/vultr-seoul-cutover.md` 신규 — `oracle-pilot.md` 5·6단계 패턴 +
  서브1~5 산출물(systemd 잡, 쿠키/CORS 설정, `.env` 이관, 백업 타이머) 통합:
  - 데이터 마이그레이션(rsync, 서브5 백업 스코프와 동일한 파일 대상)
  - go/no-go 게이트 표(KIS 커넥티비티, API헬스, 대시보드, 세션쿠키, 배치잡 타이머,
    백업 잡, 재부팅 복구)
  - 드라이런 절차(최소 3~4일, 맥 fallback 유지, 매일 확인 항목)
  - 컷오버 확정+맥종료 절차, 롤백 경로
  - 마지막 섹션에 "이 세션이 자율로 할 수 없는 것" 명시(VM 생성, rsync 실행, 게이트
    실측, 드라이런, 맥종료 판단 — 전부 VM 생성 후 사람이 진행)
- `docs/deploy/vultr-seoul.md` "다음" 목록 최종 갱신 — 서브6은 "런북 완료, 실행은
  VM 생성 후"로 표시.

**클라우드 이전 6개 서브프로젝트 전부 이 세션에서 완료 가능한 만큼 진행됨**:
1(완료, VM/nginx/systemd 킷) → 2(완료, `12c6f95`/`31c823a`) → 3(완료, `854aec3`) →
4(완료, `cf3c10e`) → 5(완료, `271191d`) → 6(런북만 완료, 실행은 VM 생성 대기).

**유저 복귀 후 필요한 것**: Vultr VM 생성(결제/SSH키) → `docs/deploy/vultr-seoul.md`
1단계부터 순서대로 진행하면 서브1~5 전부 이미 준비된 킷으로 바로 배포 가능 →
`docs/deploy/vultr-seoul-cutover.md`로 컷오버.

## 세션 로그 (2026-09-13) — 클라우드 이전 서브프로젝트5: 백업/재해복구 (`271191d`)
...
### 완료된 작업
- **`/console/insider-flow-live`, `/console/dart-events-live` 실측 엔드포인트 신설**: OpenInsider(미국 오픈마켓 매수, 캐시 7일 TTL 자동갱신 확인)·OpenDART 주요사항보고 캐시(자사주/유상증자/CB 등)를 심볼/종목코드로 필터링해 노출. 삼성전자(005930) 실측 자사주취득 공시로 동작 검증. 커밋 `3597da7`(console_api.py 쪽은 seokminal-multi-venue 별도 커밋).
- **autopilot 에이전트 STEP 3D 배선**: `tools/insider.sh`(미국)/`tools/dart.sh`(한국) 신설 + CLAUDE.md에 D관점(참고용, 매수신호 아님) 추가 — 내부자매수/자사주취득 확신가점, 유상증자/CB발행 시 경계. pytest 1975 passed 유지.

### 다음 할 일 (사용자가 이번에 새로 요청, 아직 미착수 — 아키텍처 논의 필요)
- **자본배분 구조 변경**: 라이브/페이퍼 이분법 → "전략(AI)이 필요 금액을 계좌에 청구 → 배정받은 금액 기준 PnL 산정" 모델로 재설계 요청. 자금 청구 판단에도 AI 개입 원함. **실제 계좌자금 이동이 걸린 아키텍처 변경 — brainstorming 절차 필요, 구현 전 설계 승인 필수, 실행 단계는 실거래 관련이라 자율진행 불가(사용자 명시 승인 필요).**
- **"내가 만든 모든 기능 복합 판단" 요청**: 뉴스/거시지표/회계장부(재무제표)/공시문서 등 대시보드에 존재하는 기능 전부를 에이전트 판단에 통합 원함. 현재 대시보드엔 `console_api.py`에 100개 이상 엔드포인트가 있지만 대부분 macro/insider/dart처럼 "배선 전엔 demo 데이터 or 미연결" 상태로 추정 — 전수 감사 필요. 규모상 브레인스토밍 스킬로 서브프로젝트 분해부터 필요.
...
- 모바일 실기기 push 검증: 접속 링크(`http://100.108.67.7:3000`, Tailscale) 제공 완료 — 실제 push 도착 확인은 사용자 폰에서 접속 시도 필요(다음 세션 결과 확인).
...
### 완료된 작업
- **골드 데이터소스 결정 — BLOCKED**: `ps`/`lsof`로 IB Gateway/TWS 미구동 확인(7496/7497/4001/4002 전부 리스닝 없음). `probe_ib_gold4.py`/`probe_ib_spotgold.py`도 트리에서 발견 안 됨(삭제/이동 추정). IB Gateway는 GUI+수동 2FA 필요해 자동기동 불가 — **사용자 복귀 후 수동 기동 필요**로 보류, 스킵.
- **CB/BW v3 shadow 필터 설계+등록 완료**: `research/paper/buyback_v3_dilution_forward.py` 신규 작성(`buyback_v2_forward.py` 패턴 그대로 — FROZEN_DATE 사전등록, in-sample/forward 분리). 규칙: buyback 이벤트 이전 90일 내 같은 종목 CB/BW(`cb_issue`) 발행공시 있으면 제외(희석 오버행 회피). in-sample 결과: n_v1=1785→n_v3=1748(37건 제외), net 1.603%→1.756%, 승률 50.76%→51.09% — 방향 일치하나 소폭. forward는 0건(등록 직후, FROZEN_DATE=2026-08-25). `jarvis/registry/lifecycle.py::StrategyRegistry.register()`로 `kr_buyback_v3_dilution_shadow` draft 등록(config_hash `sha256:f76f4bab...`), `research/agents/experiment_registry.py::log_experiment()`로 결과 기록. 최소 유닛테스트 `tests/test_buyback_v3_dilution_forward.py`(경계값 4케이스) 추가, 통과.
- **US 내부자매수 유니버스 확장 — 데이터 백필 완료, 재검증 진행 중**: 기존 유니버스(27개 대형주, 실제로는 `data/intraday/*_15m.parquet` 글롭 기반)가 대부분 메가캡이라 내부자매수 자체가 희소(UNDERPOWERED 확정, 2026-07-17). `research/fetch_insider_universe_expansion.py` 신규 작성 — 은행/에너지/제약/산업재/통신 등 내부자매수가 상대적으로 흔한 섹터 중형주 37종목(WFC BAC C GS MS USB PNC TFC XOM CVX COP OXY PFE MRK ABBV BMY GILD CAT DE HON GE BA MMM F GM T VZ INTC DIS KO PEP MCD TGT LOW CVS UPS FDX) Alpaca IEX 무료피드로 15m봉 900일치 백필 — 전부 성공(종목당 ~15,900~17,600봉). `run_us_insider.py` 재실행 백그라운드 진행 중(SEC EDGAR Form4 조회가 종목당 네트워크 I/O라 느림) — 결과 미확인.

### 변경된 파일
- 신규: `research/paper/buyback_v3_dilution_forward.py`, `tests/test_buyback_v3_dilution_forward.py`, `research/fetch_insider_universe_expansion.py`
- 데이터: `data/intraday/{WFC,BAC,C,GS,MS,USB,PNC,TFC,XOM,CVX,COP,OXY,PFE,MRK,ABBV,BMY,GILD,CAT,DE,HON,GE,BA,MMM,F,GM,T,VZ,INTC,DIS,KO,PEP,MCD,TGT,LOW,CVS,UPS,FDX}_15m.parquet` 신규(37개)
- registry: `jarvis/_state/registry.jsonl`에 `kr_buyback_v3_dilution_shadow` draft 1건 append, `research/agents/experiment_registry.jsonl`에 검증 결과 1건 append
...
## 2026-08-05: 함의관계 아비트라지 모듈 라이브 전환 (수집 개시)

배경: 유저가 폴리마켓 고수익 트레이더(swisstony) 프로필 조사(브라우저+API 리버스엔지니어링, `data-api.polymarket.com/positions`·`/activity` 엔드포인트, 지갑주소 `0x204f72f35326db932158cba6adff0b9a1da95e14` 확보) 후 "나도 이런 봇 만들고 싶다"고 요청. 조사 결과 해당 계정은 시간당 ~1000건 체결(5시간 5000건, MLB 1.5%뿐·축구/테니스/e스포츠가 대부분), 진입가 전구간 고른 분포 — 마켓메이킹/상관마켓 헤지형 패턴으로 판단, 새로 설계하지 말고 이미 SDD 완료돼있던 `project_polymarket_market_implication_module`(코드 완성, 라이브 미시작 상태) 재개를 제안 → 승인받음.

### 완료된 작업
- Groq `llama-3.3-70b-versatile` 모델 유효성 실호출 확인(200 OK) — deprecated 안 됨.
...
- `scripts/deploy/ensure_collectors.sh`, `api_server/lab_api.py`(`COLLECTOR_SESSIONS`)에 두 세션 등록 완료.

### 변경된 파일
- `scripts/deploy/ensure_collectors.sh`, `api_server/lab_api.py`
...
**VM 생성 시작일 결정**: 유저가 비용 문제 제기("딱 한달정도만 전부터 돈을
지불할 생각") — 컷오버는 오늘(9/13) 시작 안 해도 됨, 어차피 컷오버 이후 VM이
입대기간 내내 상시운영 떠맡으므로 "미리 켜놓는 기간"만 비용 낭비. **VM 생성은
9/19(입대 한달 전)로 확정**, 서브1(1~2일)+데이터이관/go-no-go(1일)+드라이런
(3~4일) = 6~8일 → 9/25~27 컷오버 그대로 맞음, 입대까지 3~4주 여유도 남음.

### 다음 할 일
- **9/19에 VM 생성**(사람, 결제/SSH키) → 서브1(`vultr-seoul.md`)부터 순서대로.
  그 전(9/13~18)엔 클라우드 이전 관련 추가 코드작업 없음 — 문서/코드 다 준비됨.
- `.env`에 `ANTHROPIC_API_KEY` 채우는 것도 VM 생성 시점("마지막에 키 입력할게" — 유저 확인).
