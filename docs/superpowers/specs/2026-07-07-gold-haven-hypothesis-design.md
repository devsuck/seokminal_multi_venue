...
**VERDICT: REJECT — 기준 미달**

GC 932일(2022-09-30~2026-07-01) 대상 실행 결과:
- 전략 Sharpe 0.352 vs buyhold Sharpe 1.257 — buyhold를 못 이김
- random baseline(N=200) 대비 15th percentile, p=0.85 — random과 구분 안 됨(기준: ≥95th, p<0.05)
- walk-forward 전반/후반 Sharpe 0.267 / 0.374 — 둘 다 양수(이 기준은 통과)
- cost stress(20bps) Sharpe 0.145
- underpowered 플래그: 아니오(932일 ≥ 252일)

레짐 게이트+리스크오프 부스트 메커니즘 자체는 방향은 맞았지만(WF 양쪽 양수), 무엇보다
**단순 buyhold를 못 이겼다** — 매일 체크로 인한 회전율 부담 + 게이트가 FLAT로 자산을
비우는 구간의 기회비용이 "항상 롱" 대비 우위를 상쇄한 것으로 추정(정밀 원인 분석은
안 함, 이번 버전은 기각으로 충분).

**다음 단계 결정:** 이 버전은 폐기. 스코프 밖 후보(DXY 역상관, 이벤트 트리거형, 레짐스코어
통합) 재시도는 보류 — 재시도할 근거(이번 실패의 구체적 원인)가 아직 없음. 자산 한정
(narrow-universe) 트랙 자체에 대한 결론: "금 단일자산 특화 로직"이 광역 TSMOM(이미 GC
포함, 검증통과)보다 낫다는 가설은 이번 시도에서 지지받지 못함.
