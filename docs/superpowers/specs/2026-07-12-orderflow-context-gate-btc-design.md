...
**룩어헤드 금지**: 15분봉의 트렌드/키레벨 신호는 그 봉의 종가(구간 마지막 시점)로 계산되므로, 봉이 마감되기 전(같은 구간의 60s 버킷들)에 그 신호를 적용하면 미래 정보 누출이다. broadcast는 반드시 "직전에 마감된 15분봉"의 신호를 다음 15분봉 구간의 60s 버킷들에 적용해야 한다 — 형성 중인(마감 안 된) 봉 자신의 구간에는 그 봉의 신호가 아직 적용되면 안 된다(그 구간은 그 이전 봉의 신호를 유지하거나, 이전 봉이 없으면 HOLD).
