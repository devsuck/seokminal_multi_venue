"""KR CB/BW 발행(전환사채) 이벤트 스터디 — PIT/survivorship-free (KRX 공식 스냅샷).
...
def main():
    print("=" * 74)
    print("KR CB/BW 발행(전환사채) 이벤트 스터디 — PIT/survivorship-free (KRX 공식)")
    print("=" * 74)
    cb = load_events("cb_issue")
    if not cb:
        print("cb_issue 이벤트 없음 — 먼저 pull 필요: "
              "python3 -c \"from research.data.kr_dart_events import pull_events,save_events; "
              "save_events('cb_issue', pull_events('cb_issue', years=6.5))\"")
        return
    ...
    print(f"cb_issue 이벤트 {len(cb)} / buyback(대조) {len(bb)} | random pool {len(pool)}")

    results = {}
    for name, rt in COST_LEVELS.items():
        ce = _eval(cb, series, rt)
        if not ce:
            print("cb_issue 매칭 0 (KRX 범위 밖?)"); return
        ...
        print(f"\n[{name}] cb_issue n={len(ce)} net={cnet:+.4f} vs random pct={pv['percentile']} p={pv['p_value']} (med={pv['random_median']:+.4f})")
    ...
    # 회피/음드리프트 연구: 발행이 random 하위(pct 낮음)면 "음의 드리프트 확인"(롱 회피 근거)
    neg_drift = pct <= 5.0 and base["net"] < base["med"]
    verdict = ("NEGATIVE_DRIFT 확인 — CB발행 후 random 하위 5%(롱 회피 근거, 숏 아님)" if neg_drift else
               "WEAK_NEGATIVE — random 하위지만 5%초과" if pct < 20 else
               "NO_EDGE — CB발행 드리프트 random과 구분불가")
    print(f"\nVERDICT: {verdict}")
    print("데이터: PIT + survivorship-free (KRX 스냅). 연구전용, live 매매 아님.")

    log_experiment({"hypothesis_id": "kr_cb_issuance_negdrift_v1_PIT",
                    "status": "research_negative_drift" if neg_drift else "rejected" if pct >= 20 else "weak",
