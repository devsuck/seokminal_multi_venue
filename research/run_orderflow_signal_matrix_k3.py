"""8개 오더플로우 프리미티브 3-way 전원합의 AND 스윕(C(8,3)=56/심볼, BTC+ETH=112개).
페어와이즈(2-way, 0/70 생존) 스윕에 이어 "셋으로 하면?" 질의에 대한 후속 확인.
자체 BH-FDR 풀(다른 배치와 안 섞음). 수익률/승률 순 정렬.

⚠️ DORMANT 확인용 스크립트. 112개 동시검정 스크리닝 — 실집행 근거 아님.
"""
...
def main() -> None:
    ...
    for symbol in ("BTC", "ETH"):
        ...
        if not paths:
            ...
            continue
        r = run_matrix_k(f"{symbol}.HL", paths, k=K, n_runs=N_RUNS, seed=SEED)
        if r.get("blocked"):
    ...
    print(f"\n=== 3-way AND 스윕 전체({len(all_results)}개, 유효표본 {len(valid)}개) — 수익률 순 ===\n")
    for r in sorted(valid, key=lambda x: x["strategy"]["total_pnl"], reverse=True):
        s, rnd = r["strategy"], r["random"]
        print(f"{r['symbol']}:{r['combo']:42s} trades={s['num_trades']:5d} "
